...
class HttpManager {
  ...
  Future<BaseResponse> request(
      String url, params, Options option, bool? noTip) async {
    noTip ??= false;
    ...
    try {
      response =
          await _dio.request<BaseResponse>(url, data: params, options: option);
  ...
  Future<BaseResponse> get(String url,
      {Object? params, Options? option, bool? noTip}) async {
    option ??= Options();
    option.method = "get";
